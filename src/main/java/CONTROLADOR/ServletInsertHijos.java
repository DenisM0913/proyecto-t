 ...
 * @author david
 */
@WebServlet(name = "ServletInsertHijos", urlPatterns = {"/ServletInsertHijos"})
public class ServletInsertHijos extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ServletInsertHijos</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ServletInsertHijos at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        numeroHijo = request.getParameter("numeroHijo");
...
// Verificación de existencia de idAsistencia en la solicitud
        ...
        if (idAsistenciaParam != null && !idAsistenciaParam.isEmpty()) {
            idHijo = Integer.parseInt(idAsistenciaParam);
        } else {
            idHijo = 0; // O asigna el valor que desees para indicar que no se proporcionó un id
        }
        ...
        switch (op.toUpperCase()) {
            case "GUARDAR":
                Hijo objInsert = new Hijo(cedulaOpasaporte, numeroHijo,
                        nombres, apellidos, fechaNacimiento, nivelInstrucion, usuario_idCedula);

                if (test.registrarHijo(objInsert)) {
            ...
            case "EDITAR":
                Hijo objact = new Hijo(idHijo, cedulaOpasaporte, numeroHijo,
                        nombres, apellidos, fechaNacimiento, nivelInstrucion, usuario_idCedula);
        ...
        RequestDispatcher objdevuelve = request.getRequestDispatcher("hijosInsert.jsp");
